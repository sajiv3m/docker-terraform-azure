# Definition for first Windows worker node - windcrworker2

resource "azurerm_virtual_machine" "mywindcrworker2" {
    name                  = "windcrworker2"
    location              = "${var.AZURE_LOCATION}"
    resource_group_name   = "${azurerm_resource_group.mydockergroup.name}"
    network_interface_ids = ["${azurerm_network_interface.mywindcrworker2.id}"]
    vm_size               = "Standard_DS2_v2"

#   default OS disk size for the Windows 2016 image is 127G
    storage_os_disk {
        name              = "windcrworker2-osdisk"
        caching           = "ReadWrite"
