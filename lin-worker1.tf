# Definition for first Linux worker node - lindcrworker1
resource "azurerm_virtual_machine" "mylindcrworker1" {
    name                  = "lindcrworker1"
#   This name and VM hostname,mentioned as computer_name below,should be same
    ...
    network_interface_ids = ["${azurerm_network_interface.mylindcrworker1.id}"]
    vm_size               = "Standard_DS2_v2"

#   default os-disk size is 10G, which is not enough hence mentioned as 40G
    storage_os_disk {
        name              = "lindcrworker1-osdisk"
        caching           = "ReadWrite"
